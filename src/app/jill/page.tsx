...
export default async function JillPage({
  searchParams,
}: {
  searchParams: Promise<{ e?: string; p?: string }>
}) {
  const password = process.env.JILL_GUIDE_PASSWORD
  ...
  const isAuthenticated =
    matchesGuideCookie(cookieStore.get('jill_guide_auth')?.value, password) ||
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-[#2B1F38] px-6 py-12 text-[#FCF4EB]">
      <form action="/api/jill-auth" method="post" className="w-full max-w-sm space-y-5">
