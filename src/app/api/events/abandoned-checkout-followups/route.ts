...
async function acquireEventLock(automationKey: string, eventSlug: string, mode: string, now: Date) {
  const definition = getCheckoutFollowupDefinition(automationKey as typeof ABANDONED_CHECKOUT_DAY1_AUTOMATION_KEY)
...
export async function GET(request: NextRequest) {
  ...
  const automationKey = ABANDONED_CHECKOUT_DAY1_AUTOMATION_KEY
  ...
  for (const event of await getAbandonedCheckoutEligibleEvents(now, automationKey)) {
    const hasLock = await acquireEventLock(automationKey, event.slug, control.mode, now)
